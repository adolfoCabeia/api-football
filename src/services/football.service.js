...
export class FootballService {
    static async handleRequest(endpoint) {
        try {
    ...
    static getMatchesByDate(date) {
        ...
        return this.handleRequest(`/football-get-matches-by-date?date=${date}`);
    }
    static getMatchDetail(id) {
        if (!id) {
            throw new AppError("id é obrigatório", 400);
        }
        return this.handleRequest(`/football-get-match-detail?eventid=${id}`);
    }
    static getMatchStats(id) {
        if (!id) {
            throw new AppError("id é obrigatório", 400);
        }
        return this.handleRequest(`/football-get-match-all-stats?eventid=${id}`);
    }
    static getLeagues() {
    ...
    static getTransfers(page = 1) {
        return this.handleRequest(`/football-get-all-transfers?page=${page}`);
    }
    static getLogoTeam(teamId) {
